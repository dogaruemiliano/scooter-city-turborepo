...
const DialogOverlay = ({
  ...
  return (
    <DialogPrimitive.Backdrop
      ...
      className={cn(
        "fixed inset-0 isolate z-overlay bg-scrim duration-fast ease-standard data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0",
        className,
      )}
      {...props}
    />
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Popup
        ...
        className={cn(
          "fixed top-1/2 left-1/2 z-modal grid w-full max-w-[calc(100%-var(--spacing-8))] -translate-x-1/2 -translate-y-1/2 gap-4 rounded-xl border border-border bg-popover p-4 text-sm text-popover-foreground shadow-lg duration-fast ease-standard outline-none sm:max-w-96 data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0",
          className,
        )}
        {...props}
      >
