 ...
 *
 * Rules baked in:
 * - `passwordSchema` enforces only `min(8)` (and a sane `max` to prevent
 *   bcrypt DoS via gigantic passwords). No complexity refine.
...
/**
 * Password. Min 8, max 128 (bcrypt's input limit; longer is silently
 * truncated by bcrypt anyway, so reject explicitly). No complexity rules.
 */
