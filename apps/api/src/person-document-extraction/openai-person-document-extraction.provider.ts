...
Every person suggestion must include addressEvidence. For countryCode, region, city, addressLine1, addressLine2 and postalCode, set addressEvidence to {section: "domicile" or "residence", label: the exact printed heading identifying the source address block}. All these values must come from that residential block in the referenced source image. For other person fields set addressEvidence to null. Never use birthplace, issuing-authority address, nationality, MRZ birthplace or CNP county digits as the person's address. If the domicile/residence block is absent or unreadable, omit its address fields; do not fall back to birthplace. Electronic national IDs must return no address fields; extract those from the separately supplied proof of address.
