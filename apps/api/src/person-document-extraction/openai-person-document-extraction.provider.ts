...
Driving licence issuer: inspect the country code inside the EU flag in the top-left corner of the front, as well as any printed country name. A visible "RO" or "Ro" in that flag, or the printed country name "ROMÂNIA"/"ROMANIA", means target document, field issuingCountryCode, value "RO", sourceSlot front. The country code in the flag is sufficient even without a separate country label. Do not confuse it with nationality, birthplace or address. Preserve a different visible issuing country; if the issuer is unreadable, omit it so the application can apply its workflow default.
Romanian national IDs: CNP belongs to the person, so return it as target person, field cnp, separately from document series and number. When a CNP is visible, omit dateOfBirth: the application derives it deterministically from the validated CNP. Both classic IDs and electronic IDs (CEI) have a series and a number: always return two document suggestions, field series and field number. Classic IDs print separate "SERIA" and "NR." fields: e.g. SERIA RX, NR. 001234 means series RX and number 001234. CEI prints a single "Număr document / Document number" field containing the series immediately followed by the number without a space: e.g. ZR0012345 means series ZR and number 0012345. Split the leading two letters into series and keep the remaining digits in number; never return the combined value as number. Preserve every leading zero, uppercase the series, and omit printed labels and spaces. Do not confuse the identifier with CNP, CAN (card access number), production serial number, or MRZ check digits. If either part is unreadable, return only the legible part; do not invent the missing part. For a request of type nationalId with electronic format, do not invent an address; it comes from the separate proof of address. Proof of address includes Romanian domicile/residence certificates and should supply only person/address fields, never a document number, series or expiry date.
...
Romanian addresses: return county in region and locality in city separately. Resolve printed county abbreviations (e.g. Jud.VL means Vâlcea). Mun./Municipiul, Or./Oraș/Orașul, Com./Comuna, Sat/Satul and Loc./Localitatea introduce locality names, not part of those names. The city field represents the administrative locality: a city or commune, except in București (Bucharest). For București, region must be "București" and city must be exactly "Sector 1", "Sector 2", "Sector 3", "Sector 4", "Sector 5" or "Sector 6", using only the sector printed in the domicile address. For example, "Mun. București Sectorul 3 Str.Exemplu Nr.12" means region "București", city "Sector 3", addressLine1 "Str. Exemplu, Nr. 12". Never return "București" as city or put the sector in addressLine2. If the sector is missing or unreadable, omit city; do not guess it from a street, postcode, birthplace or issuing authority. If both village and commune are printed, city is the commune; keep the village in addressLine2. The application matches county and locality against its Romanian administrative-area dataset; do not guess a commune from a village. addressLine1 must contain only the street and street number. Put village, building, staircase, floor, apartment and other premises details in addressLine2. Never put county or locality labels such as Jud.VL into either address line: resolve the county code to region and city/commune to city. Do not invent a locality from a county or infer a county from a city.
