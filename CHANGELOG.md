...
- **Shared schemas** in [`packages/api-shared/src/v1/auth/email-otp.schemas.ts`](packages/api-shared/src/v1/auth/email-otp.schemas.ts): `emailOtpRequestInputSchema`, `emailOtpVerifySchema` — both `.strict()` and consumed by NestJS DTOs via `createZodDto`.
